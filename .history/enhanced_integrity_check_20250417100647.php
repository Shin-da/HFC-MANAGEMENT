...
function safeQuery($conn, $query, $errorMessage = "Query failed") {
    try {
        ...
        echo "<div class='message issue'>Error: " . htmlspecialchars($errorMessage) . " - " . htmlspecialchars($e->getMessage()) . "</div>";
        return null;
    }
}
...
$result = safeQuery($conn, $tables_query, "Failed to get tables");
...
if ($result && $result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $count_result = safeQuery($conn, $count_query, "Failed to count records in $table_name");
        $count = 0;
        if ($count_result) {
        ...
        $structure_result = safeQuery($conn, $structure_query, "Failed to get structure for $table_name");
        
        if ($structure_result) {
            $table_structures[$table_name] = [];
            
            // Check column existence in this table
            $columns_in_table = [];
            
            while ($column = $structure_result->fetch_assoc()) {
                $table_structures[$table_name][] = $column;
                $columns_in_table[] = $column['Field'];
                ...
                if ((substr(strtolower($column_name), -2) == 'id' || 
                    ...
                    $foreign_key_candidates[] = [
                        'table' => $table_name,
                        'column' => $column_name,
                        'columns_in_table' => $columns_in_table,
...
foreach ($tables as $table_name) {
    ...
    if (isset($table_structures[$table_name]) && !empty($table_structures[$table_name])) {
        ...
        echo "<div class='message warning'>Could not retrieve structure for this table.</div>";
    }
}
echo "</div>";
...
foreach ($tables as $table) {
    if (isset($primary_keys[$table])) {
        ...
        $null_result = safeQuery($conn, $null_check_query, "Failed to check NULL values in $table.$pk");
...
foreach ($tables as $table) {
    ...
    $uk_result = safeQuery($conn, $uk_query, "Failed to get unique keys for $table");
    ...
    if ($uk_result && $uk_result->num_rows > 0) {
        while ($uk_row = $uk_result->fetch_assoc()) {
            ...
            $dup_result = safeQuery($conn, $dup_check_query, "Failed to check duplicates in $table.$column");
...
foreach ($foreign_key_candidates as $fk) {
    ...
    if (in_array($fk['potential_ref_table'], $tables)) {
        ...
        // Make sure the column actually exists in the referenced table
        $column_exists = false;
        if (isset($table_structures[$ref_table])) {
            foreach ($table_structures[$ref_table] as $column) {
                if ($column['Field'] == $ref_column) {
                    $column_exists = true;
                    break;
                }
            }
        ...
        if ($column_exists) {
            ...
            try {
                ...
                $orphan_result = safeQuery($conn, $orphan_check_query, 
                                         "Failed to check orphaned records between {$fk['table']}.{$fk['column']} and $ref_table.$ref_column");
                ...
                $status = "<span class='warning'>Error checking: " . htmlspecialchars($e->getMessage()) . "</span>";
            }
        } else {
            $status = "<span class='warning'>Referenced column not found</span>";
        }
...
foreach ($common_relationships as $rel) {
    ...
    if (in_array($rel['from_table'], $tables) && in_array($rel['to_table'], $tables)) {
        // Verify that columns exist in both tables
        $from_col_exists = false;
        $to_col_exists = false;
        
        if (isset($table_structures[$rel['from_table']])) {
            foreach ($table_structures[$rel['from_table']] as $column) {
                if ($column['Field'] == $rel['from_column']) {
                    $from_col_exists = true;
                    break;
                }
            }
        }
        
        if (isset($table_structures[$rel['to_table']])) {
            foreach ($table_structures[$rel['to_table']] as $column) {
                if ($column['Field'] == $rel['to_column']) {
                    $to_col_exists = true;
                    break;
                }
            }
        ...
        if ($from_col_exists && $to_col_exists) {
            ...
            try {
                ...
                $result = safeQuery($conn, $query, 
                                   "Failed to check orphaned records between {$rel['from_table']}.{$rel['from_column']} and {$rel['to_table']}.{$rel['to_column']}");
                ...
                if ($result) {
                    ...
                    if ($row['orphan_count'] > 0) {
                        ...
                        // Get sample of orphaned records - safely
                        try {
                            ...
                            $sample_result = safeQuery($conn, $sample_query, "Failed to get sample orphaned records");
                            ...
                            echo "<div class='message warning'>Could not retrieve orphaned record samples: " . htmlspecialchars($e->getMessage()) . "</div>";
                        }
                    } else {
                ...
                echo "<div class='message warning'>Error checking relationship: " . htmlspecialchars($e->getMessage()) . "</div>";
            }
        } else {
            if (!$from_col_exists) {
                echo "<div class='warning'>Relationship check skipped: Column '{$rel['from_column']}' not found in table '{$rel['from_table']}'.</div>";
            }
            if (!$to_col_exists) {
                echo "<div class='warning'>Relationship check skipped: Column '{$rel['to_column']}' not found in table '{$rel['to_table']}'.</div>";
            }
        }
    } else {
        if (!in_array($rel['from_table'], $tables) && !in_array($rel['to_table'], $tables)) {
...
foreach ($email_tables as $et) {
    if (in_array($et['table'], $tables)) {
        ...
        $column_exists = false;
        if (isset($table_structures[$et['table']])) {
        ...
        if ($column_exists) {
            ...
            try {
                ...
                $email_result = safeQuery($conn, $email_check_query, 
                                        "Failed to check email format in {$et['table']}.{$et['column']}");
                ...
                if ($email_result) {
                    ...
                    if ($email_row['invalid_count'] > 0) {
                        ...
                        // Get sample of invalid emails - safely
                        try {
                            ...
                            $sample_result = safeQuery($conn, $sample_query, "Failed to get invalid email samples");
                            if ($sample_result && $sample_result->num_rows > 0) {
                            ...
                            echo "<div class='message warning'>Could not retrieve invalid email samples: " . htmlspecialchars($e->getMessage()) . "</div>";
                        }
                    }
                }
            } catch (Exception $e) {
                echo "<div class='message warning'>Error checking email format: " . htmlspecialchars($e->getMessage()) . "</div>";
            }
        }
    }
}
...
echo "<ol>";
echo "<li>Review and fix any identified orphaned records, particularly in account_request and approved_account tables.</li>";
...
echo "<li>For tables with 'Referenced table not found' warnings, you may want to:
        <ul>
            <li>Create these missing reference tables if they should exist</li>
            <li>Rename the columns if they're not actually foreign keys</li>
            <li>Or just ignore these warnings if the relationships aren't critical</li>
        </ul>
      </li>";
echo "<li>Create a database backup before making any corrections to address the issues identified.</li>";
echo "<li>Set up a regular schedule for performing database integrity checks, especially after data imports or schema changes.</li>";
echo "</ol>";
