class ChatManager {
    ...
    async openChat(userId) {
        try {
            ...
            // Update chat header
            document.querySelector('.user-info .user-name').textContent = data.user.username;
            document.querySelector('.user-info .user-status').classList.toggle('online', data.user.is_online);

            // Enable input and show chat area
            const messageInput = document.querySelector('#messageInput');
            const sendButton = document.querySelector('#sendMessage');
            ...
            // Add receiver ID to the chat container
            messageInput.dataset.receiverId = userId;

            // Load chat history
            await this.loadMessages(userId);

            // Show chat view, hide welcome message
            ...
            const chatMain = document.querySelector('.chat-main');
            chatMain.classList.remove('loading');
            chatMain.innerHTML = '<div class="error">Failed to open chat</div>';
        }
    }
