...
function initializeDashboard() {
    try {
        console.log('Starting initializeDashboard function');
        console.log('window.dashboardData exists:', typeof window.dashboardData !== 'undefined');
        
        // Check if dashboardData exists globally
        if (typeof window.dashboardData !== 'undefined') {
            console.log('Using global dashboardData:', window.dashboardData);
            try {
                // Use the global dashboardData for charts
                initializeSalesCharts(window.dashboardData);
                console.log('Charts initialized successfully!');
            } catch (error) {
                console.error('Error initializing charts with dashboardData:', error);
                const debugInfo = document.createElement('div');
                debugInfo.innerHTML = `<strong>Error initializing charts:</strong> ${error.message}`;
                document.body.appendChild(debugInfo);
            }
        } else {
            console.error('dashboardData is not defined. Attempting fallback to API data.');
            
            // Check if chart elements exist
            const chartElements = {
                myChart: document.getElementById('myChart'),
                polarAreaChart: document.getElementById('polarAreaChart'),
                categoryChart: document.getElementById('categoryChart'),
                seasonalChart: document.getElementById('seasonalChart')
            };
            
            console.log('Chart elements found:', Object.entries(chartElements)
                .map(([id, element]) => `${id}: ${element ? 'Found' : 'NOT FOUND'}`));
        ...
        errorContainer.innerHTML = `<h3>Dashboard Error</h3><p>${error.message}</p>`;
        
        // Try to append to a common container or body
