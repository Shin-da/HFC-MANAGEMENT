class ChatManager {
    constructor() {
        ...
        this.activeTab = 'chat'; // Changed default tab
    ...
    initialize() {
        ...
        this.initializeTabs(); // This is now defined
    ...
    setupUnloadHandler() {
        window.addEventListener('beforeunload', async () => {
            try {
                await fetch(CONFIG.PATHS.users.offline, {
                    method: 'POST',
                    keepalive: true
                });
            } catch (error) {
    ...
    async updateOnlineStatus() {
        ...
        try {
            const response = await fetch(CONFIG.PATHS.users.online);
    ...
    async loadRecentChats() {
        try {
            const response = await fetch(CONFIG.PATHS.chat.recent);
    ...
    async openChat(userId) {
        try {
            ...
            // Get user info
            const response = await fetch(`${CONFIG.PATHS.users.info}?user_id=${userId}`);
            const data = await response.json();
    ...
    async loadMessages(userId) {
        try {
            const response = await fetch(`${CONFIG.PATHS.chat.messages}?user_id=${userId}`);
            const data = await response.json();
    ...
    async markMessagesAsRead(userId) {
        try {
            await fetch(CONFIG.PATHS.chat.markRead, {
                method: 'POST',
                headers: {
    ...
    // Add missing initializeTabs method
    initializeTabs() {
        const tabs = {
            chat: document.querySelector('.chat-tab'),
            online: document.querySelector('.online-tab')
        };

        const containers = {
            chat: document.querySelector('.chat-container'),
            online: document.querySelector('.online-users-container')
        };
        ...
        Object.keys(tabs).forEach(tabName => {
            if (tabs[tabName]) {
                tabs[tabName].addEventListener('click', () => {
                    this.switchTab(tabName, tabs, containers);
                });
            }
        });

        // Initialize with default tab
        this.switchTab(this.activeTab, tabs, containers);
    }

    switchTab(tabName, tabs, containers) {
        ...
        // Add active class to selected tab and show container
        if (tabs[tabName]) tabs[tabName].classList.add('active');
        if (containers[tabName]) containers[tabName].style.display = 'block';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof currentUser === 'undefined') {
        console.error('currentUser is not defined. Make sure user data is available.');
        return;
    }
