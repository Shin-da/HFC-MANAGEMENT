...
async function connectToDatabase() {
  try {
    const response = await fetch('./database/dbconnect.php', {
      mode: 'cors',
      headers: {
    ...
    window.db = data ? data : null;
    window.db = data ? JSON.parse(data) : null;
...
function transferOrdersToHistory() {
  ...
  if (window.db) {
    ...
    toast.fire({
      icon: 'error',
      title: 'Error',
      text: 'Database connection not established',
    });
  }
}
...
/******  ded8399b-c6e1-4090-afc5-946dd577d213  *******/