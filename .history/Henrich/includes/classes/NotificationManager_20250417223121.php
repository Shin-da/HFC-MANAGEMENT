...
class NotificationManager {
    ...
    public function clearAllNotifications($userId) {
        try {
            // Prepare statement to delete notifications for the user (excluding global user_id = 0)
            $stmt = $this->conn->prepare("DELETE FROM notifications WHERE user_id = ?");
            $stmt->bind_param("i", $userId);
            return $stmt->execute();
        } catch (Exception $e) {
            error_log("Error clearing all notifications for user ID $userId: " . $e->getMessage());
            return false;
        }
    }
} 