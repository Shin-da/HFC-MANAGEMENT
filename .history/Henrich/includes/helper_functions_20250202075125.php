...
function insertStockMovements($conn, $data) {
    $stmt = $conn->prepare("INSERT INTO stockmovement (ibdid, batchid, productcode, productname, numberofbox, totalpacks, totalweight, dateencoded, encoder) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
function insertStockActivityLog($conn, $data) {
    ...
    foreach ($data['productcode'] as $key => $code) {
        $description .= "{$data['productname'][$key]} ($code) - {$data['totalpacks'][$key]} packs\n";
    }
