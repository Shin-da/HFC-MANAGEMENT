...
function setRememberMeCookie($token) {
    ...
    $path = '/';
    $domain = 'localhost'; // Explicitly set domain for testing - can be problematic, try empty string '' if this fails
    $secure = false; // False for HTTP
    $httponly = true;

    setcookie('remember_token', $token, $expires, $path, $domain, $secure, $httponly);
    error_log("[DEBUG setRememberMeCookie] Cookie set with Domain: {$domain}, Path: {$path}, Expires: {$expires}"); // Add log
}
