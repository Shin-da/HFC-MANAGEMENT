    ...
    try {
        ...
        if ($user && password_verify($password, $user['password'])) {
            if ($user['status'] === 'active') {
                ...
                $stmt = $conn->prepare("UPDATE users SET failed_attempts = 0, last_failed_attempt = NULL WHERE useremail = ?");
                $stmt->execute([$useremail]);
                ...
                try {
                    ...
                    if ($checkTable->rowCount() > 0) {
                        $stmt = $conn->prepare("INSERT INTO activity_logs (user_id, activity_type, details) VALUES (?, 'login', 'User logged in successfully')");
            ...
            if ($user) {
                try {
                    $stmt = $conn->prepare("UPDATE users SET failed_attempts = failed_attempts + 1, last_failed_attempt = NOW() WHERE useremail = ?");
                    $stmt->execute([$useremail]);
                } catch (Exception $e) {
            ...
            try {
                ...
                if ($user) {
                    $checkTable = $conn->query("SHOW TABLES LIKE 'activity_logs'");
                    if ($checkTable->rowCount() > 0) {
                        $stmt = $conn->prepare("INSERT INTO activity_logs (user_id, activity_type, details) VALUES (?, 'failed_login', 'Failed login attempt')");
                        $stmt->execute([$user['user_id']]);
                    }
                }
            } catch (Exception $e) {
