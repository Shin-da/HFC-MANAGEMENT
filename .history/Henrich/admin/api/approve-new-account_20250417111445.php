...
require_once '../includes/config.php'; // Use PDO from config
...
try {
    ...
    // Determine role - use position/department or a default? Let's default to 'supervisor' for now
    $role = $request['requested_role'] ?? 'supervisor'; // Adjust default role as needed
    $status = 'active'; 
    ...
    // Update the status of the request in account_requests to 'processed' or delete it
    // Option 1: Update status
    $stmt_update_req = $GLOBALS['pdo']->prepare("UPDATE account_requests SET status = 'processed', processed_date = NOW(), processed_by = ? WHERE request_id = ?");
