...
// Page::addScript('https://cdn.jsdelivr.net/npm/charts.js');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const chartData = {
        sales: {
            ...
            dates: <?= json_encode($orderdate_values) ?>,
            label: <?= json_encode($chartLabel ?? "All Orders") ?>
        },
        products: {
            labels: <?= json_encode($chart_labels_chart) ?>,
            values: <?= json_encode($chart_data_chart) ?>
        },
        categories: <?= json_encode($food_analytics['category_trends']) ?>,
        seasonal: <?= json_encode($seasonal_trends) ?>
    };
