...
/*************  ✨ Codeium Command 🌟  *************/
            <div class="order-detail">
                <div class="order-header">
                    <a href="javascript:window.history.back()" id="back-to-prev" style="font-size: 3rem;">
                ...
                <div class="order-header">
                    ...
                    if (isset($_GET['hid'])) {
                        $orderId = $_GET['hid'];
                        $sql = "SELECT timeoforder FROM orderhistory WHERE hid = '$orderId'";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            $timeOfOrder = $row["timeoforder"];
                            echo "<p>Viewing Order ID # $orderId Created at: $timeOfOrder</p>";
                        } else {
                            echo "<p>Viewing Order ID # $orderId</p>";
                        }
        <div class="order-detail">
            <div class="order-header">
            ...
            <div class="order-header">
                ...
                <div class="order-body">
                    ...
                    if (isset($_GET['hid'])) {
                        $orderId = $_GET['hid'];
                        $sql = "SELECT * FROM orderhistory WHERE hid = '$orderId'";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if (isset($_SESSION['userlevel']) && $_SESSION['userlevel'] == 'admin') {
                            ...
                            <div class='order-item'>
                                ...
                                <span class='$status'>" . htmlspecialchars($status) . "</span>
                                <form action='' method='post'>
                                    <input type='hidden' name='hid' value='$orderId'>
                                    <select name='status' class='form-control'>
                                        <option value='Pending'>Pending</option>
                                        <option value='Ongoing'>Ongoing</option>
                                        <option value='Completed'>Completed</option>
                                    ...
                                    <div id='invoice' class='receipt-content'>
                                        <div class='invoice-container'>
                                            <div class='invoice-header'>
                                                ...
                                                <div class='invoice-info'>
                                                    <h2>Invoice</h2>
                                                    <p>Invoice ID: " . htmlspecialchars($orderId) . "</p>
                                                    <p>Invoice Date: " . htmlspecialchars(date('Y-m-d')) . "</p>
                                                    <p>Due Date: " . htmlspecialchars(date('Y-m-d', strtotime('+7 days'))) . "</p>
                                                    <p>Issued by: " . htmlspecialchars($_SESSION['username']) . "</p>
                                                </div>
...
/******  a2815d4c-31ce-427c-836b-eb406318101f  *******/
        </div>
    </div>
