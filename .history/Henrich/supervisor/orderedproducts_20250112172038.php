        ...
        <div class="container-fluid"> <!-- Stock Management -->
            <div class="table-header" style="border-left: 8px solid #fa1;">
                ...
                <?php // pagination for stock management table
                    ...
                    $start = ($page - 1) * $limit;
                    $items = $conn->query("SELECT * FROM orderedproduct ORDER BY orderdate DESC LIMIT $start, $limit");
            ...
            <div class="table-header">
                ...
                $limit = isset($_GET['limit']) ? $_GET['limit'] : 10;
                $start = ($page - 1) * $limit;
                $items = $conn->query("SELECT * FROM orderedproduct ORDER BY orderdate DESC, ordertime DESC LIMIT $start, $limit");
                $totalRecords = $conn->query("SELECT COUNT(*) FROM orders")->fetch_row()[0];
                $totalPages = ceil($totalRecords / $limit);
                ?>
                <div style=" display: flex; justify-content: space-around; align-items: center; width: 100%;"> <!--  Filter results by number of entries -->
...
<?php require '../reusable/footer.php'; ?>
            ...
            $stockManagementTableSQL = "SELECT * FROM orderedproduct ORDER BY orderdate DESC, ordertime DESC LIMIT $limit OFFSET $offset";
            if ($conn->connect_error) {
                die("Connection failed: " . $conn->connect_error);
            }
            $sql = $stockManagementTableSQL;
                        ...
                        <tbody id="table-body">
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    $quantity = $row['quantity'];
                                    $orderdate = date('d-m-Y', strtotime($row['orderdate']));
                                    $ordertime = date('H:i:s', strtotime($row['ordertime']));
                            ?>
                                    <tr class="clickable-row" onclick="location.href='orderdetail.php?orderid=<?= $orderid ?>'">
                                        <td><?= $orderid ?></td>
                                        <td><?= $productcode ?></td>
...
/******  c87198fe-44c5-462e-a306-7e7f4c6a96cb  *******/