    ...
    <section class=" panel">
        ...
        <div class="container-fluid"> <!-- Stock Management -->
            ...
            <div class="table-header">
                ...
                $start = ($page - 1) * $limit;
                $items = $conn->query("SELECT * FROM orderedproduct ORDER BY timeoforder DESC LIMIT $start, $limit");
                $items = $conn->query("SELECT * FROM orderedproduct ORDER BY orderdate DESC, timeoforder DESC LIMIT $start, $limit");
            ...
            $stockManagementTableSQL = "SELECT * FROM orderedproduct ORDER BY timeoforder DESC LIMIT $limit OFFSET $offset";
            $stockManagementTableSQL = "SELECT * FROM orderedproduct ORDER BY orderdate DESC, timeoforder DESC LIMIT $limit OFFSET $offset";
                        ...
                        <tbody id="table-body">
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    $timeoforder = date('H:i:s', strtotime($row['timeoforder']));
                            ?>
                                    <tr class="clickable-row" onclick="location.href='orderdetail.php?orderid=<?= $orderid ?>'">
                                        ...
                                        <td><?= $orderdate ?></td>
                                        <td><?= $timeoforder ?></td>
                                        <td><?= date('g:i A', strtotime($timeoforder)) ?></td>
                                    </tr>
                            <?php
                                }
                            } else {
...
/******  6ec83bd8-ec89-4d6d-b7e2-23e8a056432f  *******/