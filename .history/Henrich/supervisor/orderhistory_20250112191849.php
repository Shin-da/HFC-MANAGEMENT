            ...
            <div class="">
                ...
                <script>
                    function filterTable() {
                        ...
                        rows.forEach(row => {
                            const orderDate = row.querySelector('td:nth-child(7)').textContent; // assuming order date is in the 7th column
                            const orderStatus = row.querySelector('td:nth-child(6)').textContent; // assuming order status is in the 6th column
                ...
                <table class="table" id="myTable">
                    <thead>
                        <tr>
                            ...
                            <th>Order Type</th> <!-- Added column for order type -->
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $ordertype = $row['ordertype']; // Assuming 'ordertype' field exists
                                $symbol = '';
                                switch ($status) {
                                    case 'Pending':
                        ...
                        ?>
                                <tr
                                    ...
                                    <td><?= $orderdate ?></td>
                                    <td><?= $datecompleted ?></td>
                                    <td><?= $ordertype ?></td> <!-- Display order type -->
                                    <?php if ($_SESSION['role'] === 'admin') { ?>
                                    <td>
                                        <a href="edit_order.php?hid=<?= $hid ?>" class="icon-link"><i class="bx bx-edit"></i></a>
                                        <a href="delete_order.php?hid=<?= $hid ?>" class="icon-link" onclick="return confirm('Are you sure you want to delete this order?');"><i class="bx bx-trash"></i></a>
                                    </td>
                                    <?php } ?>
                                </tr>
                        <?php
                            ...
                            echo "<tr><td colspan='8' style='text-align: center;'>No records found. Please try searching by date or status.</td></tr>";
                        }
                        $conn->close();
                        ?>
                    </tbody>
                    ...
                    <div class="container" style="display: flex; justify-content: center; flex-direction: column; align-items: center;">
                            <ul class="pagination">
                                <li class="page-item <?php echo $page == 1 ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?= $page == 1 ? '#' : "?page=" . ($page - 1) . "&limit=$limit" ?>"><i class="bx bx-chevron-left bx-fw"></i></a>
                                </li>
                                ...
                                <li class="page-item <?php echo $page == $totalPages ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?= $page == $totalPages ? '#' : "?page=" . ($page + 1) . "&limit=$limit" ?>"><i class="bx bx-chevron-right bx-fw"></i></a>
                                </li>
                            </ul>
                        </div>
                    </div>
...
/******  34fb50e6-e185-4f33-9fa2-273bfa7acdca  *******/