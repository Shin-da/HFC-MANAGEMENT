...
function getStockData($conn, $cache_file, $cache_ttl, $refresh_requested) {
    ...
    try {
        ...
        // Optimize query - combine stats in one query
        $combined_query = "SELECT 
            (SELECT COUNT(*) FROM inventory) as total_products,
            (SELECT COUNT(*) FROM inventory WHERE availablequantity <= 10 AND availablequantity > 0) as low_stock,
            (SELECT COUNT(*) FROM inventory WHERE availablequantity = 0) as out_of_stock,
            (SELECT SUM(availablequantity * unit_price) FROM inventory) as total_value";

        // Get inventory data with optimized query
        ...
        // Get low stock alerts with optimized query
        $alerts_query = "SELECT 
            productcode,
            productname,
            availablequantity,
        ...
        // Optimize top products query with index hint
        ...
        $combined_stmt = $conn->prepare($combined_query);
        $inventory_stmt = $conn->prepare($inventory_query);
        $alerts_stmt = $conn->prepare($alerts_query);
        ...
        // Get results
        $stats = $combined_stmt->get_result()->fetch_assoc();
        ...
        // Optimize trends query to only get last 30 days with better index usage
        $trends_query = "SELECT 
        ...
        $trends_stmt = $conn->prepare($trends_query);
        $trends_stmt->execute();
