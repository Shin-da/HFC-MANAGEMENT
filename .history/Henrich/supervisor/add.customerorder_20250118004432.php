                    ...
                    productCodeSelect.addEventListener("change", function() {
                        ...
                        xhttp.onreadystatechange = function() {
                            if (this.readyState === 4 && this.status === 200) {
                                ...
                                var response = JSON.parse(this.responseText);
                                console.log("Query executed. Number of rows returned: " + response.numrows);
                                console.log("Available quantity: " + response.availablequantity);
                                console.log("Stock status: " + response.stockstatus);
                                document.getElementById("availablequantity").value = response.availablequantity;
                                document.getElementById("stockstatus").innerHTML = response.stockstatus;
...
/******  985a0517-df7a-4a4f-bada-1b4fa119d0a6  *******/
            ...
            <script>
                ...
                xhttp.onreadystatechange = function() {
                    ...
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        var response = this.responseText.replace('<!-- xampp local -->', '');
                        console.log(`Response text: ${response}`);
                        // Parse the JSON response
                        var response = JSON.parse(this.responseText);
                        console.log(`Parsed response: ${JSON.stringify(response)}`);
                        // Get the available quantity and stock status
                        var data = response.data;
                        console.log(`Available quantity: ${data.availablequantity}`);
                        // Update the available quantity and status icon
                        var availableQuantity = document.getElementById("quantity[]");
                        availableQuantity.value = data.availablequantity;
                
                        var stockStatus = document.getElementById("stock-status[]");
                        
                        if (data.availablequantity <= 5) {
                            stockStatus.innerHTML = "Low stock! Only " + data.availablequantity + " left.";
                            stockStatus.style.color = "red";
                        ...
                        console.error(`Error occurred while checking available quantity: ${this.status}`);
                    }
                };
            </script>
    </section>
