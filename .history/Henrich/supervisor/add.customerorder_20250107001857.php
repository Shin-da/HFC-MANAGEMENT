                ...
                function addTableRow() {
                    ...
                    ['productcode', 'productname', 'quantity', 'quantityType', 'productprice', 'totalprice'].forEach((key, i) => {
                        ...
                        if (key === 'quantityType') {
                            ...
                            select.innerHTML = `<option value="" disabled selected>ProductCode</option>${Object.keys(productcodes).map(k => `<option value="${productcodes[k]}">${productcodes[k]}</option>`).join('')}`;
                            ...
                            select.addEventListener("change", function() {
                                ...
                                xhttp.onreadystatechange = function() {
                                    if (this.readyState === 4 && this.status === 200) {
                                        var response = JSON.parse(this.responseText);
                                        document.getElementById(`productname${newRow.rowIndex}`).value = response.productname;
                                        document.getElementById(`productprice${newRow.rowIndex}`).value = response.productprice;
                                    }
                                };
                                xhttp.open("GET", "get_product_info.php?productcode=" + productCode, true);
                                xhttp.send();
...
/******  f8da28cf-59d5-4c4f-b9bd-279783fa471d  *******/
