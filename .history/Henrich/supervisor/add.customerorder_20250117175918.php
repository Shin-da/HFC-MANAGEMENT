        ...
        <div>
                        ...
                        <table id="customerOrderTable">
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        <td>
                                            <input name="productcode[]" id="productcode<?= $i ?>" row-id="<?= $i ?>" class="select-search" required>
                                                <option value="" disabled selected>ProductCode</option>
                                                <?php foreach ($productcodes as $productcode): ?>
                                                    <?php
                                                    $sql = "SELECT productname, productweight FROM productlist WHERE productcode='$productcode'";
                                                    $result = $conn->query($sql);
                                                    $row = $result->fetch_assoc();
                                                    ?>
                                                    <option value="<?= $productcode ?>"><?= $productcode ?> (<?= $row['productname'] ?>, <?= $row['productweight'] ?>Kg)</option>
                                                <?php endforeach; ?>
                                            </input>
                                        </td>
                                        ...
                                        <td>
                    <span id="stock-status<?= $i ?>" style="font-weight: bold;"></span>
                </td>
            ...
            <script>
              document.addEventListener("DOMContentLoaded", function() {
                  var productCodeSelect = document.getElementById("productcode<?= $i ?>");
                      ...
                      xhttp.onreadystatechange = function() {
                          if (this.readyState === 4 && this.status === 200) {
                              ...
                              if (response.error) {
                                  ...
                                  var stockStatus = document.getElementById("stock-status<?= $i ?>");
