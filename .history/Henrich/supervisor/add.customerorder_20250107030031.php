...
/*************  ✨ Codeium Command 🌟  *************/
                                        <td>
                                            <select name="productcode[]" id="productcode<?= $i ?>" row-id="<?= $i ?>" class="select-search" onchange="fetchProductInfo(this)" required style="width: 150px;">
...
/******  10ead3b5-dbeb-43e8-a0ff-f83bef115d16  *******/
                ...
                function addTableRow() {
                    ...
                    ['productcode', 'productname', 'quantity', 'quantityType', 'productprice', 'totalprice'].forEach((key, i) => {
                        ...
                        if (key === 'quantityType') {
                            ...
                            var select = document.createElement('select');
                            select.name = `${key}[]`;
                            select.id = `productcode${rowIndex}`;
                            select.rowId = rowIndex;
                            select.onchange = function() {
                                getProductInfo(this);
                            }
                            // Add options to the select element
                            // You can populate this from a database or a list of product codes
                            // For now, I'll just add a few options
                            select.innerHTML = "<option value=''>Select Product Code</option><option value='001'>Product 1</option><option value='002'>Product 2</option><option value='003'>Product 3</option>";
                            // Add a search input to the select element
                            var searchInput = document.createElement('input');
                            searchInput.type = 'search';
                            searchInput.placeholder = 'Search Product Code';
                            ...
                            newCell.appendChild(searchInput);
                            newCell.appendChild(select);
                    ...
                    xmlhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            if (productNameElement && productPriceElement) {
                                ...
                                console.error("HTML elements with IDs 'productname" + rowId + "' and 'productprice" + rowId + "' not found in the DOM");
                            }
                        }
                    };
