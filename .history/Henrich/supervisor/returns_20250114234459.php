/*************  ✨ Codeium Command 🌟  *************/
<?php
include_once '../database/dbconnect.php';
require '../reusable/redirect404.php';
require '../session/session.php';

if (isset($_POST['submit'])) {
    $order_id = $_POST['order_id'];
    $reason = $_POST['reason'];
    $message = $_POST['message'];
    $user_id = $_SESSION['user_id'];

    $query = "INSERT INTO returns (order_id, user_id, reason, message) VALUES ('$order_id', '$user_id', '$reason', '$message')";
    $result = mysqli_query($conn, $query);

    if ($result) {
        header('Location: returns.php?success=1');
        exit;
    } else {
        header('Location: returns.php?error=1');
        exit;
    }
}

if (isset($_GET['success']) && $_GET['success'] == 1) {
    $success = 'Thank you for submitting your return request. We will contact you soon.';
} elseif (isset($_GET['error']) && $_GET['error'] == 1) {
    $error = 'Something went wrong. Please try again.';
}
?>
<!DOCTYPE html>
<html>
<head>
    <title>Returns</title>
</head>
<body>

<h2>Returns</h2>

<?php if (isset($success)) { ?>
    <div style="color: green;"><?php echo $success; ?></div>
<?php } elseif (isset($error)) { ?>
    <div style="color: red;"><?php echo $error; ?></div>
<?php } ?>

<form action="returns.php" method="post">
    <label>Order ID:</label>
    <select name="order_id" required>
        <option value="">Select</option>
        <?php
            $query = "SELECT hid, customername, orderdescription, ordertotal, status FROM customerorder";
            $query = "SELECT hid, orderdescription FROM customerorder";
            $result = mysqli_query($conn, $query);
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<option value="' . $row['hid'] . '">' . $row['orderdescription'] . ' - ' . $row['customername'] . '</option>';
                    echo '<option value="' . $row['hid'] . '">' . $row['orderdescription'] . '</option>';
                }
            }
        ?>
    </select><br><br>

    <div id="order-details"></div>

    <label>Reason:</label>
    <select name="reason" required>
        <option value="">Select</option>
        <option value="Bad quality">Bad quality</option>
        <option value="Wrong item">Wrong item</option>
        <option value="Other">Other</option>
    </select><br><br>

    <label>Message:</label>
    <textarea name="message" required></textarea><br><br>

    <input type="submit" name="submit" value="Submit">
</form>

<script>
    document.querySelector('select[name="order_id"]').addEventListener('change', function() {
        var orderId = this.value;
        if (orderId) {
            fetch('get_order_details.php?order_id=' + orderId)
                .then(response => response.json())
                .then(data => {
                    var detailsDiv = document.getElementById('order-details');
                    detailsDiv.innerHTML = '<h3>Order Details</h3>' +
                        '<p>Customer Name: ' + data.customername + '</p>' +
                        '<p>Order Description: ' + data.orderdescription + '</p>' +
                        '<p>Order Total: $' + data.ordertotal + '</p>' +
                        '<p>Status: ' + data.status + '</p>';
                });
        } else {
            document.getElementById('order-details').innerHTML = '';
        }
    });
</script>

</body>
</html>


/******  a242c440-978a-47cd-8a06-ed2d97498609  *******/