...
function handleDeleteRequest($notificationManager, $userId) {
    // Check for action parameter
    $action = isset($_GET['action']) ? $_GET['action'] : 'delete_single';

    if ($action === 'clear_all') {
        // Clear all notifications for the current user
        $success = $notificationManager->clearAllNotifications($userId);
        if ($success) {
            echo json_encode(['success' => true]);
        } else {
            http_response_code(500);
            echo json_encode(['error' => 'Failed to clear all notifications']);
        }
    } elseif ($action === 'delete_single') {
        // Get notification ID from URL parameter for single delete
        $notificationId = isset($_GET['id']) ? (int)$_GET['id'] : null;
        
        if (!$notificationId) {
            http_response_code(400);
            echo json_encode(['error' => 'Missing notification ID for single delete']);
            return;
        }
        
        // Attempt to delete the single notification
        // We need to ensure the user has permission, typically by checking user_id
        // Note: The current deleteNotification doesn't check user ID, which might be a security risk.
        // For now, we proceed as is, but this should be reviewed.
