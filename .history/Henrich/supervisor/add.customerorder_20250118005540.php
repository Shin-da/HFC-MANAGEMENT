...
<!-- HTML -->
<input type="number" id="availablequantity<?= $i ?>" name="availablequantity[]" placeholder="Available Quantity" readonly>
<input type="text" id="stock-status<?= $i ?>" name="stock-status[]" placeholder="Stock Status" readonly>
      ...
      if (this.readyState === 4) {
        ...
        if (this.status === 200) {
          try {
            ...
            document.getElementById("stock-status" + rowId).value = "Low stock!";
            if (response.availablequantity > 5) {
              document.getElementById("stock-status" + rowId).value = "In stock!";
            }
          } catch (e) {
