        ...
        <div>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" name="availablequantity[]" id="availablequantity<?= $i ?>" placeholder="Available Quantity" readonly>
                                        </td>
                                        <td><input type="text" name="stockstatus[]" id="stockstatus<?= $i ?>" placeholder="Stock Status" readonly></td>
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    console.log("Product code select element found");
                            ...
                            if (this.readyState === 4) {
                                ...
                                if (this.status === 200) {
                                    try {
                                        ...
                                        var availableQuantity = document.getElementById("quantity[]");
                                        availableQuantity.value = response.availablequantity;
                                        if (response.error) {
                                            alert(response.error);
                                        } else {
                                            var availableQuantity = response.availablequantity;
                                            var quantityInput = document.getElementById("quantity[]");
                                            var stockStatus = document.getElementById("stock-status[]");

                                            quantityInput.value = availableQuantity;
                                            if (availableQuantity <= 5) {
                                                stockStatus.innerHTML = "Low stock! Only " + availableQuantity + " left.";
                                        ...
                                        if (e instanceof SyntaxError) {
                                            ...
                                            console.error(`Error parsing response: ${e.name}: ${e.message}`);
                                        }
                                    }
                                } else {
  ...
  document.getElementById("productcode<?= $i ?>").addEventListener("change", function() {
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState === 4 && this.status === 200) {
        ...
        var availableQuantityInput = document.querySelector(`#availablequantity${productCode}`);
        var stockStatusInput = document.querySelector(`#stockstatus${productCode}`);
        if (response && response.availablequantity !== undefined) {
          availableQuantityInput.value = response.availablequantity;
        }
        if (response && response.stockstatus !== undefined) {
    ...
    xhttp.send();
            ...
            <script>
                ...
                function addTableRow() {
                    ...
                    var columns = ['productcode', 'productname', 'productprice', 'productweight', 'quantity', 'availablequantity', 'stockstatus', 'totalweight', 'totalprice'];
...
make the values of the availablequantity and stockstatus show up on input field, datas show up on console but dont populate the field when a product is selected.  on the stock status it now shows undefined but nothing on the availablequantity.

html elements:
<input type="number" name="availablequantity" id="availablequantity<?= $i ?>" placeholder="Available Quantity" readonly>

<input type="text" name="stockstatus" id="stockstatus<?= $i ?>" placeholder="Stock Status" readonly>

console:
Checking available quantity for product code: 012
Query executed. Number of rows returned: 1
Available quantity: 1202
Stock status: In stock!
{"availablequantity":1202,"stockstatus":"In stock!"}
xhttp.onreadystatechange @ add.customerorder.php:1125
XMLHttpRequest.send
(anonymous) @ add.customerorder.php:1136
handleMouseUp_ @ unknown
add.customerorder.php:1049  The specified value "undefined" cannot be parsed, or is out of range.