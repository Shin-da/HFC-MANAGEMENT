                ...
                function searchProduct() {
                    ...
                    xmlhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            document.getElementById('productcode').innerHTML = productOptions;
                        }
                    };
                    xmlhttp.open("GET", "get_product_info.php?search=" + searchQuery, true);
                    xmlhttp.send();
                }
                var rowIndex = 0;
...
/*************  ✨ Codeium Command 🌟  *************/
                function addTableRow() {
                    var selectedProduct = document.getElementById('productcode').value;
                    var xmlhttp = new XMLHttpRequest();
                    xmlhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            var response = JSON.parse(this.responseText);
                            var newRow = document.getElementById('customerOrderTable').insertRow();
                            var cells = new Array(7).fill(null).map(() => newRow.insertCell());
                            cells[0].innerHTML = response.productname;
                            cells[1].innerHTML = response.productprice;
                            cells[2].innerHTML = response.productweight;
                            cells[3].innerHTML = '<input type="number" name="quantity[]" min="1" value="1" onchange="calculateTotal(this)">';
                            cells[4].innerHTML = '<input type="text" name="totalweight[]" readonly>';
                            cells[5].innerHTML = '<input type="text" name="totalprice[]" readonly>';
                            cells[6].innerHTML = '<button type="button" class="btn btn-danger" onclick="deleteTableRow(this)">X</button>';
                    ...
                    xmlhttp.open("GET", "get_product_info.php?productid=" + selectedProduct, true);
                    xmlhttp.send();
...
/******  5f040378-a5f1-4b87-9afb-72691de9a4f2  *******/
