          ...
          document.getElementById("productcode<?= $i ?>").addEventListener("change", function() {
            ...
            xhttp.onreadystatechange = function() {
              if (this.readyState === 4 && this.status === 200) {
                ...
                availableQuantityInput.value = response.availablequantity ? response.availablequantity : '';
                stockStatusInput.value = response.stockstatus ? response.stockstatus : '';
                // Dispatch an input event to trigger any event listeners
                availableQuantityInput.dispatchEvent(new Event('input'));
                stockStatusInput.dispatchEvent(new Event('input'));
              }
            };
            xhttp.open("GET", "./process/check_available_quantity.php?productcode=" + productCode, true);
            xhttp.send();
...
/******  cfd71041-79be-4694-bce5-2e2c52983fd8  *******/
