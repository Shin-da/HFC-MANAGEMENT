...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (is_array($oid) || is_array($customername) || is_array($customeraddress) || is_array($customerphonenumber) || is_array($orderdescription) || is_array($ordertotal) || is_array($orderdate) || is_array($salesperson) || is_array($status)) {
        ...
        foreach ($oid as $key => $value) {
            $oids[$key] = $value;
            $customerNames[$key] = $customername[$key];
            $customerAddresses[$key] = $customeraddress[$key];
            $customerPhoneNumbers[$key] = $customerphonenumber[$key];
            $orderDescriptions[$key] = $orderdescription[$key];
            $orderTotals[$key] = $ordertotal[$key];
            $orderDates[$key] = $orderdate[$key];
            $salespersons[$key] = $salesperson[$key];
        ...
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("sssssssss", $oids, $customerNames, $customerAddresses, $customerPhoneNumbers, $orderDescriptions, $orderTotals, $orderDates, $salespersons, $statuses);
            $stmt->execute();
            foreach ($oid as $key => $value) {
                $stmt->bind_param("sssssssss", $oid[$key], $customername[$key], $customeraddress[$key], $customerphonenumber[$key], $orderdescription[$key], $ordertotal[$key], $orderdate[$key], $salesperson[$key], $status[$key]);
                $stmt->execute();
            }
...
/******  c5f055e5-3d68-4afc-91b0-8df7af28c8a5  *******/