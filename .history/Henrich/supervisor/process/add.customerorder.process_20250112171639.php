...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO orderedproduct (orderid, productcode, productname, productweight, productprice, quantity, orderdate, timeoforder, ordertype) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt = $conn->prepare($sql)) {
        foreach ($productname as $key => $value) {
            ...
            $stmt->bind_param("issddsss", $orderIDValue, $productCodeValue, $value, $productWeightValue, $productPriceValue, $quantityValue, $orderdate, $timeoforder, $ordertype);
    ...
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("sssssssss", $orderdescription, $orderdate, $customername, $customeraddress, $customerphonenumber, $ordertotal, $salesperson, $status, $timeoforder, $ordertype);
...
/******  ffc579d1-4d7f-43ce-8e16-89e44147690e  *******/