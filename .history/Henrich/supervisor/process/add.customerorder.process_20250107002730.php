...
<table class='output-table'>
    <tr>
        <td><?= is_array($oid) ? 'Array' : htmlspecialchars($oid) ?></td>
        <td><?= is_array($customername) ? 'Array' : htmlspecialchars($customername) ?></td>
        <td><?= is_array($customeraddress) ? 'Array' : htmlspecialchars($customeraddress) ?></td>
        <td><?= is_array($customerphonenumber) ? 'Array' : htmlspecialchars($customerphonenumber) ?></td>
        <td><?= is_array($orderdescription) ? 'Array' : htmlspecialchars($orderdescription) ?></td>
        <td><?= is_array($ordertotal) ? 'Array' : htmlspecialchars($ordertotal) ?></td>
        <td><?= is_array($orderdate) ? 'Array' : htmlspecialchars($orderdate) ?></td>
        <td><?= is_array($salesperson) ? 'Array' : htmlspecialchars($salesperson) ?></td>
        <td><?= is_array($status) ? 'Array' : htmlspecialchars($status) ?></td>
        <th>OID</th>
        <th>Customer Name</th>
...
/******  9e024695-9de9-4cc1-a853-c02195dad837  *******/