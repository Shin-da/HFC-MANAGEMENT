...
try {
    ...
    $genericDescription = "Stock received for batch " . $batchId;
    $logSql = "INSERT INTO stockactivitylog (
        batchid, dateofarrival, dateencoded, encoder, 
        totalNumberOfBoxes, overalltotalweight, activity_type, description
    ) VALUES (?, ?, ?, ?, ?, ?, 'IN', ?)";
...
$totalPacks = $_POST['totalpieces']; 
$totalWeights = $_POST['totalweight'];

for ($i = 0; $i < count($productCodes); $i++) {
    // Get product name from database
    $prodNameStmt = $conn->prepare("SELECT productname FROM products WHERE productcode = ?");
    $prodNameStmt->bind_param("s", $productCodes[$i]);
    $prodNameStmt->execute();
    $prodNameResult = $prodNameStmt->get_result();
    $productName = ($prodNameResult->num_rows > 0) ? $prodNameResult->fetch_assoc()['productname'] : 'Unknown Product';
    
    // Insert into stockmovement
    $movementSql = "INSERT INTO stockmovement (
        batchid, productcode, productname, numberofbox, 
