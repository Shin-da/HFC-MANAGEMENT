...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("ississsss", $oid, $customername, $customeraddress, $customerphonenumber, $orderdescription, $ordertotal, $orderdate, $salesperson, $status);
        $stmt->execute();
...
/*************  ✨ Codeium Command ⭐  *************/
// This error typically occurs when there is an issue with a message passing between background and content scripts in a browser extension.
// To handle such errors, you can add a check for runtime.lastError in your JavaScript code.

if (chrome.runtime.lastError) {
    console.error("Error: ", chrome.runtime.lastError.message);
} else {
    // Continue with the logic that should happen after a successful message passing
}

/******  5e78481c-a9aa-4189-8759-620dff6e301c  *******/