-- Database Schema Improvement Script for Henrich Food Corps
...
ADD UNIQUE INDEX `idx_email` (`usermail`),
...
INSERT INTO users (username, usermail, role, password, first_name, last_name, status, created_at, updated_at)
...
WHERE NOT EXISTS (
    SELECT 1 FROM users u WHERE u.username = ar.username OR u.usermail = ar.usermail
)
...
INSERT INTO users (username, usermail, first_name, last_name, status, created_at, updated_at)
SELECT aa.username, aa.usermail, aa.first_name, aa.last_name, 'active', aa.created_at, aa.updated_at
FROM approved_account aa
WHERE NOT EXISTS (
    SELECT 1 FROM users u WHERE u.username = aa.username OR u.usermail = aa.usermail
)
