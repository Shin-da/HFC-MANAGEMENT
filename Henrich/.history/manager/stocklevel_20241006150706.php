...
<body>
  ...
  <?php // stock alert
  $items = $conn->query("SELECT * FROM inventory WHERE Available_quantity <= 10");
  $count = $items->num_rows;
  $alert = $count > 0 ? "alert" : "noalert";
  ?>

  <?php
  if ($alert == "alert") {
    echo "<script> alert('There is/are " . $count . " item(s) that have low stock levels!'); </script>";
  }
  ?>
