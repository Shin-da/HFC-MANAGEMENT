...
// fetch the incoming data for the total boxes and add it to the existing onhand products
$ProductCode = $_POST['productcode'][0];

$stmt = $conn->prepare("UPDATE inventory SET totalboxes = ?, totalweight = ?, totalcost = ? WHERE ProductCode = ?");
$stmt->bind_param("iiss", $totalboxes, $totalweight, $totalcost, $ProductCode);
if (!$stmt->execute()) {
    echo "Error updating data in inventory table: " . $stmt->error;
    exit;
} else {
    echo "Data updated successfully in inventory table!";
}
?>