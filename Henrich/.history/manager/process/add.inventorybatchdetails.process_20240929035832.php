...
$stmt = $conn->prepare("INSERT INTO inventoryhistory (batchid, dateofarrival ,encoder, encodeddate, description, totalboxes, totalweight, totalcost ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("isiiiss", $batchid, date('Y-m-d H:i:s'),  $_SESSION['username'], date('Y-m-d H:i:s') , 'Batch Added', $totalboxes, $totalweight, $totalprice);
if (!$stmt->execute()) {
    echo "Error inserting data: " . $stmt->error;
    exit;
}
