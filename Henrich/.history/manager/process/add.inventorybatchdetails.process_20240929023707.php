...
$stmt = $conn->prepare("INSERT INTO inventoryhistory (batchid, product, quantity, weight, price, encodedby, dateencoded) VALUES (?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("iiiiiss", $batchid, $productcode, $quantity, $weight, $price, $_SESSION['username'], date('Y-m-d H:i:s'));
if (!$stmt->execute()) {
