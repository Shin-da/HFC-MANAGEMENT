...
$stmt = $conn->prepare("SELECT COUNT(*) as count FROM stockactivityl WHERE batchid = ?");
...
if ($row['count'] > 0) {
    ...
    $stmt = $conn->prepare("UPDATE stockmovement SET totalboxes = ?, totalweight = ?, totalcost = ?, dateencoded = ? WHERE batchid = ?");
    $stmt->bind_param("iiiss", $totalboxes, $totalweight, $totalcost, $dateencoded, $batchid);
    if (!$stmt->execute()) {
        echo "Error updating data in stockmovement table: " . $stmt->error;
        exit;
    } else {
        echo "Data updated successfully in stockmovement table!";
    }
} else {
    // Batchid does not exist, insert a new row
    $stmt = $conn->prepare("INSERT INTO stockmovement (batchid, totalboxes, totalweight, totalcost, dateencoded) VALUES (?, ?, ?, ?, ?)");
