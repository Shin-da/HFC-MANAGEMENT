...
// Insert Data into inventorybatch table
$stmt = $conn->prepare("INSERT INTO inventorybatchdetails (batchid, totalcost) VALUES (?, ?)");
...
foreach ($productcodes as $key => $productcode) {
    ...
    $price = $prices[$key];
    $stmt->bind_param("isiii", $batchid, $pr3oductcode, $quantity, $weight, $price);
    $stmt->execute();
}
