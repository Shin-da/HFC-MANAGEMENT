...
$stmt = $conn->prepare("INSERT INTO inventory (productcode, productdescription, category, onhand, dateupdated) VALUES (?, ?, ?, ?, ?)");
$stmt->bind_param("ssssi", $productcode, $product['productdescription'], $product['category'], $quantity, $dateupdated);
if (!$stmt->execute()) {
    echo "Error inserting data: " . $stmt->error;
    exit;
}
