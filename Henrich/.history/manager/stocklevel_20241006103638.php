...
<section class="container-fluid"> <!-- === graph showing stock levels === -->

<?php // graph showing stock levels
  $items = $conn->query("SELECT * FROM inventory");
  $totalRecords = $items->num_rows;
  $totalPages = ceil($totalRecords / 10);
  $page = isset($_GET['page']) ? $_GET['page'] : 1;
  $start = ($page - 1) * 10;
  $items = $conn->query("SELECT * FROM inventory LIMIT $start, 10");
