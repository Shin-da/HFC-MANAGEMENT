...
function getLowStockProducts() {
    $sql = "SELECT * FROM inventory WHERE onhand <= 10";
    $result = $conn->query($sql);
    $lowStockProducts = array();
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $lowStockProducts[] = $row['productcode'];
        }
    }
          ...
          <tbody id="table-body">
            ...
            ?>
                <tr>
                  ...
                  <td><?= $category ?></td>
                  <td><?= $onhand ?>
                    <?php if (in_array($productcode, $lowStockProducts)) { ?>
...
/******  410458b3-4433-4896-baff-1907f7b7beb5  *******/