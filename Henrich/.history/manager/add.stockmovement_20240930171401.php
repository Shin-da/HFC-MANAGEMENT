                    ...
                    <table id="inventorybatchdetails">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="productcode[]" id="productcode<?= $i ?>" list="productcodes" placeholder="Product Code" onchange="fetchProductInfo()">
                                </td>
                                <td><input type="number" name="quantity[]" id="quantity<?= $i ?>" placeholder="Quantity"> </td>
/*************  ✨ Codeium Command 🌟  *************/
                               <td> <input type="number" name="weight[]" id="weight<?= $i ?>" placeholder="Weight" oninput="fetchData(this, 'weight')"></td>
                                <td><input type="number" name="price[]" id="price<?= $i ?>" placeholder="Price" oninput="fetchData(this, 'price')"></td>

                            <script>
                                function fetchData(element, type) {
                                    var productcode = document.getElementById("productcode" + element.id.split("weight")[1]).value;
                                    var xhttp = new XMLHttpRequest();
                                    xhttp.onreadystatechange = function() {
                                        if (this.readyState == 4 && this.status == 200) {
                                            var response = JSON.parse(this.responseText);
                                            if (type == 'weight') {
                                                element.value = response.weight;
                                            } else if (type == 'price') {
                                                element.value = response.price;
                                            }
                                        }
                                    };
                                    xhttp.open("GET", "../database/fetchProductInfo.php?productcode=" + productcode, true);
...
/******  cfc34b68-dd9a-4490-994d-a0663b27941b  *******/
        ...
        <script>
            ...
            function addTableRow() {
                ...
                cell2.innerHTML = `<input type="text" name="batchid[]" id="batchid${table.rows.length - 1}" placeholder="Batch ID">`;
