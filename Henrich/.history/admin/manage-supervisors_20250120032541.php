...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch($action) {
        case 'add':
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, useremail, first_name, last_name, department, role, password) VALUES (?, ?, ?, ?, ?, 'supervisor', ?)");
            $stmt->bind_param("ssssss", $username, $email, $fname, $lname, $_POST['department'], $hashed_password);
            
            if ($stmt->execute()) {
                // Send email with credentials
                mail($email, "Account Created", "Your account has been created. Username: $username, Temporary password: $temp_password");
                $success = "Supervisor added successfully";
            } else {
                $error = "Error adding supervisor";
            }
            break;
    }
}
