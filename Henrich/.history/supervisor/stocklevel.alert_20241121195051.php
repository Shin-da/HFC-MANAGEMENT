...
<?php
  ...
  if ($items->num_rows > 0) {
    ...
    while ($row = $items->fetch_assoc()) {
      ...
      $legend = "";
      if ($quantity == 0) {
        $legend = "<span style='color: red; font-weight: bold; font-size: 1.2em'>OUT OF STOCK</span>";
        $legend = "<span style='color: red'>OUT OF STOCK</span>";
      } else if ($quantity <= 5) {
        $legend = "<span style='color: orange'>LOW STOCK</span>";
      }
      echo '<li style="background-color: ' . ($quantity == 0 ? '#ff3737' : '') . '"><span class="product-name">' . $row['productname'] . '</span> - <span class="quantity">' . $row['onhand'] . '</span> units left ' . $legend . '</li>';
...
/******  1efa0e9b-7bb6-44a3-8c74-5bcbb2549575  *******/