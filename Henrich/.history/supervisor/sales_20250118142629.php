...
$enhanced_analytics = [
    ...
    'temporal_analysis' => $conn->query("
        ...
        SELECT 
            ...
            avg_order_value,
            unique_customers,
            total_sales / order_count as efficiency_score,
            order_count / unique_customers as customer_frequency_ratio
        FROM HourlyStats
        ORDER BY total_sales DESC
    ")->fetch_all(MYSQLI_ASSOC)
];
...
function calculateSalesPrediction($historical_data, $current_value) {
    $weights = array_reverse(range(1, count($historical_data))) / array_sum(range(1, count($historical_data)));
    $weighted_sum = 0;
    
    foreach ($historical_data as $index => $value) {
        $weighted_sum += $value * $weights[$index];
    }
    
    $trend_factor = ($current_value / $weighted_sum);
    $prediction = $weighted_sum * $trend_factor;
    
    return [
        'predicted_value' => $prediction,
        'confidence_score' => min(100, (1 - abs($prediction - $current_value) / $current_value) * 100),
        'trend_indicator' => $trend_factor > 1 ? 'upward' : 'downward',
        'volatility' => stats_standard_deviation($historical_data) / $weighted_sum
    ];
}
...
<body>
  ...
  <section class="panel">
    ...
    <div class="stat-card highlight">
        <div class="stat-value"><?= number_format($enhanced_analytics['growth_metrics']['growth_rate'], 1) ?>%</div>
  ...
  <script>
    ...
    new Chart(peakHoursCtx, {
        type: 'line',
        data: {
            labels: <?= json_encode(array_column($enhanced_analytics['temporal_analysis'], 'hour_of_day')) ?>,
            datasets: [{
                label: 'Sales by Hour',
                data: <?= json_encode(array_column($enhanced_analytics['temporal_analysis'], 'total_sales')) ?>,
