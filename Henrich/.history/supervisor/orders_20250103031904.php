...
<script>
    $(document).ready(function() {
        $('#product_code, #product_name').change(function() {
...
// get stocklevel info first on database table inventory
$inventorySQL = "SELECT * FROM inventory";
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}
$result = $conn->query($inventorySQL);
$inventory = array();
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $inventory[$row['itemid']] = $row['stocklevel'];
    }
}
    ...
    <section class=" panel">
        ...
        <div class="container-fluid"> <!-- Stock Management -->
            ...
            <?php // pagination for stock management table
                ...
                $stockManagementTableSQL = "SELECT * FROM orders ORDER BY oid ASC LIMIT $limit OFFSET $offset";
                if ($conn->connect_error) {
                    die("Connection failed: " . $conn->connect_error);
                }
...
make a download function and button. first get the data from the database, then downloads the records when the button is clicked
...
/******  7f32b44e-3f18-4c69-a8ee-65c7d68f81d3  *******/