...
$analytics = [
    ...
    'sales_growth' => $conn->query("
        SELECT 
            ...
            LAG(SUM(ordertotal)) OVER (ORDER BY orderdate) as prev_month_sales,
            ROUND(((SUM(ordertotal) - LAG(SUM(ordertotal)) OVER (ORDER BY orderdate)) / 
                LAG(SUM(ordertotal)) OVER (ORDER BY orderdate) * 100), 2) as growth_rate
        FROM customerorder
    ...
    'product_prediction' => $conn->query("
        SELECT 
            p.productname,
            COUNT(o.orderid) as order_frequency,
            AVG(oi.quantity) as avg_quantity,
            STDDEV(oi.quantity) as std_dev,
            p.quantity as current_stock,
            p.reorderpoint,
            ROUND(p.quantity / (AVG(oi.quantity) * COUNT(o.orderid) / 30), 0) as estimated_days_until_reorder
        FROM productlist p
        LEFT JOIN orderitems oi ON p.productid = oi.productid
        LEFT JOIN customerorder o ON oi.orderid = o.orderid
        WHERE o.orderdate >= DATE_SUB(CURRENT_DATE, INTERVAL 30 DAY)
        GROUP BY p.productid
        HAVING estimated_days_until_reorder <= 30
        ORDER BY estimated_days_until_reorder ASC
    ")->fetch_all(MYSQLI_ASSOC),
