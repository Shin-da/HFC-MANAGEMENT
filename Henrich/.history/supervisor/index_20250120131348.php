...
// Get low stock count
// $query = "SELECT COUNT(*) as count FROM inventory WHERE availablequantity <= reorder_level AND quantity > 0";
// $result = $conn->query($query);
// if ($result && $row = $result->fetch_assoc()) {
//     $dashboard_data['metrics']['low_stock_count'] = $row['count'];
// }
...
// $result = $conn->query($query);
// if ($result && $row = $result->fetch_assoc()) {
//     $dashboard_data['metrics']['out_of_stock_count'] = $row['count'];
// }

// Get today's orders and revenue
// $query = "SELECT COUNT(*) as count, COALESCE(SUM(ordertotal), 0) as total FROM customerorder WHERE DATE(orderdate) = '$today'";
// $result = $conn->query($query);
// if ($result && $row = $result->fetch_assoc()) {
//     $dashboard_data['metrics']['today_orders'] = $row['count'];
//     $dashboard_data['metrics']['today_revenue'] = $row['total'];
// }

// Get today's online orders
// $query = "SELECT COUNT(*) as count, COALESCE(SUM(ordertotal), 0) as total FROM customerorder WHERE DATE(orderdate) = '$today' AND order_type = 'online'";
// $result = $conn->query($query);
// if ($result && $row = $result->fetch_assoc()) {
//     $dashboard_data['metrics']['today_online_orders'] = $row['count'];
//     $dashboard_data['metrics']['today_online_revenue'] = $row['total'];
// }

// Initialize recent orders array
...
// Add core styles
Page::addStyle('/assets/css/supervisor.css');

// Add specific supervisor stylesheets - make sure paths are correct relative to web root
Page::addStyle('/supervisor/css/supervisor-layout.css');
Page::addStyle('/supervisor/css/supervisor-dashboard.css');

// Add scripts
Page::addScript('/assets/js/supervisor.js');
Page::addScript('/supervisor/js/supervisor-dashboard.js');
