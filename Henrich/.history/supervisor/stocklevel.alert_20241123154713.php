...
if ($outOfStockItems->num_rows > 0 && $show_stock_alert) {
  while ($row = $outOfStockItems->fetch_assoc()) {
    ...
    echo "<div class='alert-item out-of-stock-item'>";
    echo "<i class='bx bx-error'></i>";
    echo "<div class='details'>";
    echo "<span style='color: var(--sidebar-color)'><strong>OUT OF STOCK </strong> </span> - <span class='product-name'>" . $row['productname'] . "</span>";
    echo "</div>";
...
if ($items->num_rows > 0 && $show_stock_alert) {
  while ($row = $items->fetch_assoc()) {
    ...
    if ($quantity > 0) {
      ...
      echo "<div class='details'>" . $legend . " - <span class='product-name'>" . $row['productname'] . "</span> - <span class='quantity'>" . $quantity . "</span> units left";
      echo "</div>";
      echo "<span class='close' onclick='this.parentElement.style.display = \"none\"'><i class='bx bx-x'></i></span>";
      echo "</div>";
      echo "</div>";
...
/******  b92365c0-2377-4d94-9b5a-fc55b8d8dac8  *******/