...
$(document).ready(function() {
    function initializeSelect2(element) {
        $(element).select2({
            ...
            ajax: {
                ...
                processResults: function(data) {
                    console.log('Select2 data:', data); // Debug log
                    if (data.error) {
            ...
            // Automatically fill in other fields
            $('#productname' + rowId).val(data.productname);
            $('#productweight' + rowId).val(data.productweight);
            $('#productcategory' + rowId).val(data.productcategory);
        ...
        return $(`
            <div class='select2-result-product'>
                ...
                <div class='select2-result-product__meta'>
                    ${product.productcategory} | ${product.productweight}kg/pc | ${product.piecesperbox}pcs/box
                </div>` : ''}
            </div>
        `);
    }
    ...
    function formatProductSelection(product) {
        return product.id ? `${product.id} - ${product.text.split(' - ')[1] || ''}` : product.text;
    }
