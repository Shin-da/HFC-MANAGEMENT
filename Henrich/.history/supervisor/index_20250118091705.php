...
$dashboard_data = [
    ...
    'metrics' => $conn->query("
        SELECT 
            (SELECT COUNT(*) FROM productlist WHERE quantity <= reorderpoint) as low_stock_count,
...
$analytics = [
    ...
    'customer_insights' => $conn->query("
        SELECT 
            c.customerid,
            COUNT(co.orderid) as order_count,
            SUM(co.ordertotal) as total_spent
...
$dashboard_analytics = [
    ...
    // Updated inventory alerts
    'inventory_alerts' => $conn->query("
        SELECT 
            COUNT(CASE WHEN availablequantity <= reorderpoint THEN 1 END) as low_stock,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(salesCtx, {
        type: 'line',
        data: {
            labels: <?php echo json_encode(array_column($dashboard_analytics['daily_trends'], 'date')); ?>,
            datasets: [{
                label: 'Daily Sales',
                data: <?php echo json_encode(array_column($dashboard_analytics['daily_trends'], 'daily_sales')); ?>,
                borderColor: '#4CAF50',
                ...
                label: 'Orders',
                data: <?php echo json_encode(array_column($dashboard_analytics['daily_trends'], 'order_count')); ?>,
                borderColor: '#2196F3',
