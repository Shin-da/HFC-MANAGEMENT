    ...
    <script>
        ...
        function initializeSelect2(element) {
            $(element).select2({
                    ...
                    processResults: function(data) {
                        return {
                            results: data.results.map(product => ({
                                ...product,
                                text: `${product.id} - ${product.productname} (${product.availablequantity} in stock)`
                            }))
                        };
                    }
                },
                ...
                $(`#productprice${rowId}`).val(data.productprice);
                $(`#productweight${rowId}`).val(data.productweight);
                $(`#availablequantity${rowId}`).val(data.availablequantity);
                $(`#stockstatus${rowId}`).val(getStockStatus(data.availablequantity));
            });
        }
        ...
        function formatProduct(product) {
            ...
            const status = getStockStatus(product.availablequantity);
            const color = getStockColor(product.availablequantity);
        ...
        function formatProduct(product) {
            ...
            const stockColor = getStockStatusColor(product.availablequantity);
            return $(`
                <div class="select2-result-product">
                    <div class="product-info">
        ...
        // Initialize all select2 elements on document ready
        ...
        function getStockStatusColor(availableQuantity) {
            if (availableQuantity <= 0) return 'red';
            if (availableQuantity < 5) return 'orange';
            return 'green';
        }
    </script>
    <script>
