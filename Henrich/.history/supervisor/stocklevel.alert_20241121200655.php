...
<?php
  ...
  if ($outOfStockItems->num_rows > 0) {
    ...
    while ($row = $outOfStockItems->fetch_assoc()) {
      echo '<li><span class="product-name">' . $row['productname'] . '</span> - <span style="color: white">OUT OF STOCK</span></li>';
    }
    echo '</ul>';
    echo '</div>';
  }
  ...
  if ($items->num_rows > 0) {
    ...
    while ($row = $items->fetch_assoc()) {
      $quantity = $row['onhand'];
      if ($quantity > 0) {
        $legend = $quantity <= 5 ? "<span style='color: orange'>LOW STOCK</span>" : "";
...
/******  a4da713d-88f6-4791-9a07-0df802567463  *******/