/*************  ✨ Codeium Command 🌟  *************/
<style>
  .alert,
  .out-of-stock-alert {
    margin: 10px;
    background-color: var(--sidebar-color);
    width: 100%;
    border-bottom: 1px solid var(--orange-color);
    z-index: 9999;
    display: flex;
    flex-direction: column; /* Change to column for individual item display */
    flex-direction: row;
  }

  .out-of-stock-alert {
    border-bottom: 1px solid var(--accent-color);
  }

  .alert-item {
  .alert-content,
  .out-of-stock-content {
    display: flex;
    align-items: center;
    background-color: var(--orange-color);
    padding: 10px;
    justify-content: space-between;
    margin-bottom: 5px; /* Add margin between items */
  }

  .out-of-stock-item {
  .out-of-stock-content {
    background-color: #DD5746;
  }

  .alert-item h4,
  .out-of-stock-item h4 {
  .alert-content h4,
  .out-of-stock-content h4 {
    margin-top: 0;
    font-weight: bold;
    color: #DD5746;
  }

  .out-of-stock-item h4 {
  .out-of-stock-content h4 {
    color: var(--accent-color);
  }

  .alert-item i,
  .out-of-stock-item i {
  .alert-content i,
  .out-of-stock-content i {
    margin-right: 5px;
    font-size: 2em;
  }

  .alert-item .product-name,
  .out-of-stock-item .product-name {
  .alert-content .product-name,
  .out-of-stock-content .product-name {
    font-weight: bold;
  }

  .alert-item .close,
  .out-of-stock-item .close {
  .alert-content .close,
  .out-of-stock-content .close {
    background-color: var(--sidebar-color);
    border: 1px solid #fff;
    cursor: pointer;
    border-radius: 2px;
    float: right;
    font-size: 1.5em;
    display: inline-block;
    width: 30px;
    height: 30px;
    text-align: center;
    position: absolute;
    right: 10px;
  }

  .alert-item .close:hover,
  .out-of-stock-item .close:hover {
  .alert-content .close:hover,
  .out-of-stock-content .close:hover {
    background-color: #fff;
    color: var(--accent-color);
  }

  .alert-item p,
  .out-of-stock-item p {
  .alert-content p,
  .out-of-stock-content p {
    background-color: var(--sidebar-color);
    padding: 10px;
  }
</style>

<?php
require '../database/dbconnect.php';

$items = $conn->query("SELECT * FROM inventory WHERE onhand <= 10 ORDER BY onhand ASC");
$outOfStockItems = $conn->query("SELECT * FROM inventory WHERE onhand = 0 ORDER BY productname ASC");

if (isset($_COOKIE['show_stock_alert'])) {
  $show_stock_alert = $_COOKIE['show_stock_alert'];
} else {
  $show_stock_alert = true;
}

if ($outOfStockItems->num_rows > 0 && $show_stock_alert) { ?>
  <div class="out-of-stock-alert">
    <?php while ($row = $outOfStockItems->fetch_assoc()) { ?>
      <div class="out-of-stock-item">
        <i class="bx bx-error"></i>
        <span class="product-name"><?= $row['productname'] ?></span> - <span style="color: var(--accent-color)">OUT OF STOCK</span>
        <span class="close" onclick="this.parentElement.style.display = 'none'"><i class="bx bx-x"></i></span>
  <div class="  out-of-stock-alert">
    <div class="out-of-stock-content">
      <i class="bx bx-error"></i><span class="close" onclick="this.parentElement.parentElement.style.display = 'none'"><i class="bx bx-x"></i></span>
      <h4><i class="bx bx-exclamation" style="color: red"></i> Out of Stock Alert!</h4>
      <span class="product-name">
    </div>
    <p>
      <?php
      while ($row = $outOfStockItems->fetch_assoc()) {
        echo '<span class="product-name">' . $row['productname'] . '</span> - <span style="color: var(--accent-color)">OUT OF STOCK</span><br />';
      }
      echo '</p>';
      echo '</div>';
    }
    if ($items->num_rows > 0 && $show_stock_alert) { ?>
    <div class="alert">
      <div class="alert-icon"><i class="bx bx-error-circle"> </i></div>
      <div class="alert-content">
        <h4> Low Stock Alert!</h4>
        <span class="close" onclick="this.parentElement.parentElement.style.display = 'none'">
          <i class="bx bx-x"></i>
        </span>

      </div>
    <?php } ?>
  </div>
<?php }
      <p>
        <?php
        while ($row = $items->fetch_assoc()) {
          $quantity = $row['onhand'];
          $legend = '';
          if ($quantity > 0) {
            if ($quantity <= 5) {
              $legend = "<span style='color: orange'>VERY LOW STOCK</span>";
            } else {
              $legend = "<span style='color: yellow'>LOW STOCK</span>";
            }
            echo '<span class="product-name">' . $row['productname'] . '</span> - <span class="quantity">' . $quantity . '</span> units left ' . $legend . '<br />';
          }
        } ?>
      </p>  
    </div>
  <?php }

if ($items->num_rows > 0 && $show_stock_alert) { ?>
  <div class="alert">
    <?php while ($row = $items->fetch_assoc()) {
      $quantity = $row['onhand'];
      $legend = '';
      if ($quantity > 0) {
        if ($quantity <= 5) {
          $legend = "<span style='color: orange'>VERY LOW STOCK</span>";
        } else {
          $legend = "<span style='color: yellow'>LOW STOCK</span>";
        } ?>
        <div class="alert-item">
          <i class="bx bx-error-circle"></i>
          <span class="product-name"><?= $row['productname'] ?></span> - <span class="quantity"><?= $quantity ?></span> units left <?= $legend ?>
          <span class="close" onclick="this.parentElement.style.display = 'none'"><i class="bx bx-x"></i></span>
        </div>
      <?php }
    } ?>
  </div>
<?php }

if (isset($_POST['close_stock_alert'])) {
  setcookie('show_stock_alert', false, time() + (86400 * 30), '/');
}
?>
    if (isset($_POST['close_stock_alert'])) {
      setcookie('show_stock_alert', false, time() + (86400 * 30), '/');
    }
  ?>
/******  62193b82-7637-4cdf-989d-e0bcbeae97d8  *******/