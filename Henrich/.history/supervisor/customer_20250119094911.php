...
$sql3 = "SELECT * FROM customerdetails ORDER BY customerid DESC LIMIT ? OFFSET ?";
    ...
    <script>
        ...
        function showToast(message, type = 'success') {
            Toastify({
                ...
                onClick: function(){} // Prevents console errors
            }).showToast();
        }
        ...
        async function syncCustomers() {
            ...
            try {
                ...
                if (data.success) {
                    showToast(`Successfully synchronized ${data.affected} customers`);
                    setTimeout(() => {
                        window.location.reload();
                    }, 1000);
                } else {
                    showToast(data.message || 'Error synchronizing customers', 'error');
                }
            } catch (error) {
                console.error('Sync error:', error);
                showToast('Error synchronizing customers: ' + error.message, 'error');
            } finally {
        ...
        async function syncCustomers() {
            ...
            try {
                ...
                if (data.success) {
                    showToast('Customers synchronized successfully');
                    location.reload(); // Reload to show updated data
                } else {
                    showToast(data.message, 'error');
                }
            } catch (error) {
                showToast('Error synchronizing customers', 'error');
            } finally {
                showLoading(false);
            }
        }
