...
if (isset($_POST['updateStatus'])) {
    ...
    $status = $_POST['status'] ?? '';
    if ($status != '') {
        $sql = "UPDATE orders SET status = '$status', datecompleted = NOW() WHERE hid = '$hid'";
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Order status has been updated!');</script>";
            header("Location: orderdetail.php?oid=$hid");
            exit;
        } else {
...
<body>
    ...
    <div class="panel">
        ...
        <div class="order-detail">
            ...
            <div class="order-header">
                <p>Viewing Order ID #
                    <?= htmlspecialchars(isset($_GET['hid']) ? $_GET['hid'] : 'N/A') ?>
                </p>
            </div>
