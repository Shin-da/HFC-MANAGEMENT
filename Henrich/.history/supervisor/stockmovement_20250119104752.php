...
// Modify the insert query to match the actual table structure
$stmt = $conn->prepare("
    INSERT INTO stockmovement 
        (batchid, productcode, productname, numberofbox, totalpieces, totalweight, dateencoded) 
    VALUES (?, ?, ?, ?, ?, ?, NOW())
    ON DUPLICATE KEY UPDATE
        numberofbox = numberofbox + VALUES(numberofbox),
        totalpieces = totalpieces + VALUES(totalpieces),
        totalweight = totalweight + VALUES(totalweight)
");

// Modify the validation function to match actual columns
function validateMovement($productCode, $numberofbox) {
    global $conn;
    $result = $conn->query("
    ...
    if (!$stock || $numberofbox > $stock['availablequantity']) {
...
<body>
    ...
    ?>
    <section class="panel inventory-theme"><!-- === STOCK MOVEMENT === -->
        ...
        <div class="container-fluid">
            ...
            <div class="alert-config">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="lowStockThreshold" class="form-control"
                        value="<?= isset($_COOKIE['lowStockThreshold']) ? $_COOKIE['lowStockThreshold'] : 10 ?>">
                </div>
                <button onclick="saveAlertSettings()" class="btn btn-primary">Save Settings</button>
            </div>
        </div>
        <div class="container-fluid"> <!-- === STOCK MOVEMENT === -->
            ...
            <div class="container-fluid">
                ...
                <div class="stats-container">
                    ...
                    <div class="stat-card">
                        <h3>Total Boxes</h3>
                        <div class="value"><?= $stats['total_boxes'] ?></div>
                    </div>
                    <div class="stat-card">
                        <h3>Total Pieces</h3>
                        <div class="value"><?= $stats['total_pieces'] ?></div>
                    </div>
                    <div class="stat-card">
                        <h3>Total Weight</h3>
                        <div class="value"><?= number_format($stats['total_weight'], 2) ?> kg</div>
                    </div>
                </div>
...
<?php include_once("../reusable/footer.php"); ?>
    ...
    <script>
        function saveAlertSettings() {
            ...
            // Check current stock levels against threshold
            const boxes = <?= $stats['total_boxes'] ?>;
            if (boxes <= threshold) {
                alert(`Warning: Current stock (${boxes} boxes) is below or at threshold (${threshold} boxes)`);
            }
        }

        // Auto-update stats every 5 minutes
        setInterval(() => {
            fetch('get_stock_stats.php')
                .then(response => response.json())
                .then(stats => {
                    document.querySelector('.stat-card:nth-child(1) .value').textContent = stats.total_records;
                    document.querySelector('.stat-card:nth-child(2) .value').textContent = stats.total_boxes + ' boxes';
                    document.querySelector('.stat-card:nth-child(3) .value').textContent = stats.total_pieces + ' pcs';
                    document.querySelector('.stat-card:nth-child(4) .value').textContent = stats.total_weight + ' kg';
                });
        }, 300000);
    </script>
</body>
