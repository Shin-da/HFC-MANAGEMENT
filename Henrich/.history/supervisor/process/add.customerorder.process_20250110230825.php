...
/*************  ✨ Codeium Command 🌟  *************/
    $productcode = isset($row['productcode']) ? $row['productcode'] : '';
    $productname = isset($row['productname']) ? $row['productname'] : '';
    $weight = isset($row['productweight']) ? $row['productweight'] : '';
    $price = isset($row['productprice']) ? $row['productprice'] : '';
    $quantity = isset($row['quantity']) ? $row['quantity'] : '';
    $orderdate = isset($row['orderdate']) ? date('d-m-Y', strtotime($row['orderdate'])) : '';
...
/******  28a2bb32-467c-4c4f-9cf4-f66f616b61b7  *******/
    ...
    if (!empty($productname)) {
        foreach ($productname as $key => $value) {
            ...
            $orderTotalValue = isset($ordertotal[$key]) ? $ordertotal[$key] : 'N/A';
            $orderdescription .= "Product Code: $productCodeValue, Product: $value, Quantity: $quantityValue $quantityTypeValue, Price: $productPriceValue, Total: $orderTotalValue\n";
        }
    }
