...
erts        $dateencodeds = array_fill(0, count($productcodes), date('Y-m-d H:i:s')); // Changed to include time
        ...
        foreach ($productcodes as $key => $productcode) {
            ...
            $stmt = $conn->prepare("INSERT INTO stockmovement (ibdid, batchid, productcode, productname, numberofbox, totalpieces, totalweight, encoder, dateencoded) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("iissiiis", $ibdid, $batchid, $productcode, $productname, $numberofbox, $totalpieces, $totalweight, $encoder);
