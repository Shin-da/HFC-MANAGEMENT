    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            foreach ($productcode as $code) {
                $categoryStmt->bind_param("s", $code);
                $categoryStmt->execute();
                $result = $categoryStmt->get_result();
                if ($row = $result->fetch_assoc()) {
                    $productCategories[$code] = $row['productcategory'];
                }
            }
            
            // Insert into orderlog with product categories
            $orderLogSql = "INSERT INTO orderlog (orderid, productcode, productname, productweight, unit_price, quantity, orderdate, timeoforder, productcategory) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($orderLogStmt = $conn->prepare($orderLogSql)) {
                ...
                foreach ($productcode as $key => $code) {
                    ...
                    $category = $productCategories[$code] ?? 'Uncategorized';
                ...
                require_once 'create_order_notification.php';
                createOrderNotification($orderid, $customername, $ordertotal);
