...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($productcodes as $key => $productcode) {
        if (empty($productcode) || empty($encoders[$key] ?? '') || empty($quantitiesperboxs[$key]) || empty($quantityperpieces[$key]) || empty($totalweightss[$key] ?? '')) {
            $hasEmptyValue = true;
            break;
        }
    }
    ...
    foreach ($productcodes as $key => $productcode) {
        ...
        $stmt = $conn->prepare("INSERT INTO stockmovement (ibdid, batchid, encoder, productcode, quantity, totalweight, dateencoded) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt = $conn->prepare("INSERT INTO stockmovement (ibdid, batchid, encoder, productcode, quantity, totalpieces, dateencoded) VALUES (?, ?, ?, ?, ?, ?, ?)");

        // Bind the parameters
        $stmt->bind_param("sssssii", $ibdid, $batchid, $encoder, $productcode, $quantity, $totalweight, $dateencoded);
        $stmt->bind_param("sssssii", $ibdid, $batchid, $encoder, $productcode, $quantity, $totalpieces, $dateencoded);
...
/******  9d701260-9bd3-46bb-b1ad-7e9823ac92db  *******/