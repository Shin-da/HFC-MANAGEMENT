...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($productcodes as $key => $productcode) {
        if (empty($productcode) || empty($productnames[$key]) || empty($totalpiecess[$key]) || empty($totalweightss[$key])) {
    ...
    foreach ($productcodes as $key => $productcode) {
        ...
        $stmt = $conn->prepare("INSERT INTO stockmovement (ibdid, batchid, productcode, productname, totalpieces, totalweight, encoder, dateencoded) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssisssi", $ibdid, $batchid, $productcode, $productname, $totalpieces, $totalweight, $encoder, $dateencoded);
        ...
        $stmt->bind_param("ssssisssi", $ibdid, $batchid, $productcode, $productname, $numberofbox, $totalpieces, $totalweight, $encoder, $dateencoded);
...
/******  976f456d-033f-4072-81b4-65a381ddeb56  *******/