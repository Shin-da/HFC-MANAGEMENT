...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ = $_POST['quantityperbox'] ?? []; // quantity per box
    ...
    foreach ($productcodes as $key => $productcode) {
        if (empty($productcode) || empty($productnames[$key]) || empty($quantitiesofboxs[$key]) || empty($totalpiecess[$key]) || empty($totalweightss[$key])) {
            $hasEmptyValue = true;
            break;
        }
    }
    ...
    foreach ($productcodes as $key => $productcode) {
        ...
        $quantityofbox = $quantitiesofboxs[$key] ?? '';
        ...
        $stmt = $conn->prepare("INSERT INTO stockmovement (ibdid, batchid, productcode, productname, quantityofbox, totalpieces, totalweight, encoder, dateencoded) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssss", $ibdid, $batchid, $productcode, $productname, $quantityofbox, $totalpieces, $totalweight, $encoder, $dateencoded);
