...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $encoders = array_fill(0, count($productcodes), $encoder);

    $dateencoded = $_POST['dateencoded'] ?? '';
    $dateencodeds = array_fill(0, count($productcodes), $dateencoded);
    ...
    foreach ($productcodes as $key => $productcode) {
        echo "<tr>";
        echo "<td>" . (isset($ibdids[$key]) ? (string)$ibdids[$key] : '') . "</td>";
        echo "<td>" . (isset($ibdids[$key]) ? $ibdids[$key] : '') . "</td>";
        echo "<td>" . $batchid . "</td>";
        echo "<td>" . $productcode . "</td>";
        echo "<td>" . (isset($productnames[$key]) ? (string)$productnames[$key] : '') . "</td>";
        echo "<td>" . (isset($numberofboxes[$key]) ? (string)$numberofboxes[$key] : '') . "</td>";
        echo "<td>" . (isset($totalpiecess[$key]) ? (string)$totalpiecess[$key] : '') . "</td>";
        echo "<td>" . (isset($totalweightss[$key]) ? (string)$totalweightss[$key] : '') . "</td>";
        echo "<td>" . (isset($encoders[$key]) ? (string)$encoders[$key] : '') . "</td>";
        echo "<td>" . (isset($dateencodeds[$key]) ? (string)$dateencodeds[$key] : '') . "</td>";
...
/******  a32b8a8a-b2cf-4ec7-a92c-c3c3853f54ed  *******/
