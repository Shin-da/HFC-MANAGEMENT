...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($productcodes as $key => $productcode) {
        $ibdid = (int)($ibdids[$key] ?? 0);
        $productname = (string)($productnames[$key] ?? '');
        $numberofbox = (int)($numberofboxes[$key] ?? 0);
        $totalpieces = (int)($totalpiecess[$key] ?? 0);
        $totalweight = (float)($totalweightss[$key] ?? 0.0);
        $ibdid = $ibdids[$key] ?? '';
        ...
        $encoder = (string)($encoders[$key] ?? '');
        $dateencoded = (string)($dateencodeds[$key] ?? '');
...
/******  749b2601-8436-4ddf-84ba-9960117ac7e5  *******/