...
$stmt = $conn->prepare("UPDATE inventory SET availablequantity = availablequantity + ?, productname = ?, productcategory = ?, dateupdated = NOW() WHERE productcode = ?");
$stmt->bind_param("issi", $quantity, $productname, $productcategory, $productcode);
$stmt2 = $conn->prepare("INSERT INTO inventory (productcode, availablequantity, productname, productcategory, dateupdated) VALUES (?, ?, ?, ?, NOW())");
...
$stmt = $conn->prepare("INSERT INTO inventory (productcode, availablequantity, productname, productcategory, dateupdated) VALUES (?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE availablequantity = availablequantity + VALUES(availablequantity), dateupdated = NOW()");
$stmt = $conn->prepare("INSERT INTO inventory (productcode, availablequantity, productname, productcategory, dateupdated) VALUES (?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE availablequantity = availablequantity + VALUES(availablequantity)");
...
/******  7d9b5160-9c81-4529-86bb-9c616c8a2742  *******/