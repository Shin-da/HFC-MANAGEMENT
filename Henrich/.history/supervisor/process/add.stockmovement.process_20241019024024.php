...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($productcodes as $key => $productcode) {
        ...
        echo "<tr>";
        echo "<td>" . $ibdids[$key] ?? '' . "</td>";
        echo "<td>" . $batchid . "</td>";
        echo "<td>" . $productcode . "</td>";
        echo "<td>" . $productnames[$key] ?? '' . "</td>";
        echo "<td>" . $numberofboxes[$key] ?? '' . "</td>";
        echo "<td>" . $totalpiecess[$key] ?? '' . "</td>";
        echo "<td>" . $totalweightss[$key] ?? '' . "</td>";
        echo "<td>" . $encoders[$key] ?? '' . "</td>";
        echo "<td>" . $dateencodeds[$key] ?? '' . "</td>";
        echo "</tr>";
    }
    if ($hasEmptyValue) {
...
/******  5541ce3f-00d1-4b39-a342-5f0c6075b633  *******/
    $hasEmptyValue = false;
    foreach ($productcodes as $key => $productcode) {
        if (empty($productcode) || empty($productnames[$key]) || empty($numberofboxes[$key]) || empty($totalpiecess[$key]) || empty($totalweightss[$key])) {
            $hasEmptyValue = true;
