    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($orderLogStmt = $conn->prepare($orderLogSql)) {
                ...
                // Create notification for new order with debug logging
                try {
                    error_log("=== Starting Order Notification Creation ===");
                    ...
                    $notificationResult = createOrderNotification(
                        $orderid, 
                        $customername, 
                        $ordertotal, 
                        'new_order'
                    ...
                    if ($notificationResult) {
                        error_log("Successfully created order notification");
                    } else {
                        error_log("Failed to create order notification");
                    }
