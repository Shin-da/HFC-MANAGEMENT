...
// Add a session check to prevent infinite redirects
if (!isset($_SESSION['processing_product'])) {
    $_SESSION['processing_product'] = true;
} else {
    // If we're already processing, something's wrong
    unset($_SESSION['processing_product']);
    header("Location: products.php?error=redirect_loop");
    exit();
}
...
try {
    ...
    // Validate product ID format (assuming it's alphanumeric)
    ...
    // Clear the processing flag once we have the product
    unset($_SESSION['processing_product']);
