...
$query_recommendations = "
    ...
    SELECT 
        ...
        CASE
            WHEN ordertotal < previous_sale THEN 'Declining'
            WHEN ordertotal > previous_sale THEN 'Growing'
            ELSE 'Stable'
...
function generateRecommendation($metrics) {
    ...
    // Price optimization
    if ($metrics['ordertotal'] < $metrics['avg_product_revenue']) {
        $optimal_price = $metrics['avg_product_revenue'] * 1.1;
    ...
    if ($metrics['trend_direction'] === 'Declining') {
        if ($metrics['performance_category'] === 'High Performer') {
    ...
    switch ($metrics['performance_category']) {
        case 'High Performer':
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_assoc($result_recommendations)) {
                    $metrics = [
                        ...
                        'avg_product_revenue' => $row['avg_product_revenue'],
                    ...
                    if ($row['performance_category'] === 'High Performer' && $row['trend_direction'] === 'Declining') {
                        $priority_class = 'high-priority';
                        $priority_level = 'High';
                    } elseif ($row['performance_category'] === 'Needs Attention') {
                    ...
                    echo "<td class='trend-{$row['trend_direction']}'>{$row['trend_direction']}</td>";
                    echo "<td>" . number_format($row['ordertotal'], 2) . " (" . 
                         ($row['ordertotal'] > $row['avg_product_revenue'] ? '+' : '') . 
                         number_format(($row['ordertotal'] - $row['avg_product_revenue']) / $row['avg_product_revenue'] * 100, 1) . 
                         "%)</td>";
