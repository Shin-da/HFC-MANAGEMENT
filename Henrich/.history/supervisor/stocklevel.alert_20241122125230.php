...
<?php
  ...
  if ($items->num_rows > 0 && $show_stock_alert) { ?>
    ...
    echo '<div class="alert-icon"><i class="bx bx-error-circle" style="color:#ffffff"> </i></div>';
...
if ($outOfStockItems->num_rows > 0 && $show_stock_alert) {
  $stockAlertsHtml .= '<div class="out-of-stock-alert">';
  $stockAlertsHtml .= '<div class="out-of-stock-content">';
  $stockAlertsHtml .= '<i class="bx bx-error"></i><span class="close" onclick="this.parentElement.parentElement.style.display = \'none\'"><i class="bx bx-x"></i></span>';
  $stockAlertsHtml .= '<h4><i class="bx bx-exclamation" style="color: red"></i> Out of Stock Alert!</h4>';
  $stockAlertsHtml .= '</div>';
  $stockAlertsHtml .= '<p>';

  while ($row = $outOfStockItems->fetch_assoc()) {
    $stockAlertsHtml .= '<span class="product-name">' . $row['productname'] . '</span> - <span style="color: var(--accent-color)">OUT OF STOCK</span><br />';
  }
  $stockAlertsHtml .= '</p>';
  $stockAlertsHtml .= '</div>';
}
...
if ($items->num_rows > 0 && $show_stock_alert) {
  $stockAlertsHtml .= '<div class="alert">';
  $stockAlertsHtml .= '<div class="alert-icon"><i class="bx bx-error-circle" style="color:#ffffff"> </i></div>';
  $stockAlertsHtml .= '<div class="alert-content">';
  $stockAlertsHtml .= '<h4> Low Stock Alert!</h4>
  <span class="close" onclick="this.parentElement.parentElement.style.display = \'none\'">
  <i class="bx bx-x"></i>
  </span>';
  ...
  while ($row = $items->fetch_assoc()) {
    ...
    if ($quantity > 0) {
      ...
      $stockAlertsHtml .= '<span class="product-name">' . $row['productname'] . '</span> - <span class="quantity">' . $quantity . '</span> units left ' . $legend . '<br />';
    }
  }
  $stockAlertsHtml .= '</p>';
  $stockAlertsHtml .= '</div>';
...
echo $stockAlertsHtml;

/******  65e296fd-5614-4276-bd53-973fbd031458  *******/