...
<?php
  ...
  if ($outOfStockItems->num_rows > 0) {
    ...
    while ($row = $outOfStockItems->fetch_assoc()) {
      echo '<li style="border-left: 5px solid var(--accent-color)"><span class="product-name">' . $row['productname'] . '</span> - <span style="color: var(--accent-color)">OUT OF STOCK</span></li>';
    ...
    while ($row = $items->fetch_assoc()) {
      ...
      if ($quantity > 0) {
        if ($quantity <= 5) {
          ...
          echo '<li style="border-left: 5px solid orange"><span class="product-name">' . $row['productname'] . '</span> - <span class="quantity">' . $quantity . '</span> units left ' . $legend . '</li>';
        } else {
          $legend = "<span style='color: yellow'>LOW STOCK</span>";
          echo '<li style="border-left: 5px solid yellow"><span class="product-name">' . $row['productname'] . '</span> - <span class="quantity">' . $quantity . '</span> units left ' . $legend . '</li>';
        }
...
/******  775308e1-f0fb-4eff-9314-41e866bdc9ee  *******/