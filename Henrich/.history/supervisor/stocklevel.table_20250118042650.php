<?php
// Pagination setup
$page = isset($_GET['page']) ? $_GET['page'] : 1;
$limit = isset($_GET['limit']) ? $_GET['limit'] : 10;
$start = ($page - 1) * $limit;
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'inventoryid';
$order = isset($_GET['order']) ? $_GET['order'] : 'ASC';

// Get total records for pagination
$totalRecords = $conn->query("SELECT COUNT(*) FROM inventory")->fetch_row()[0];
$totalPages = ceil($totalRecords / $limit);

// Fetch inventory data
$items = $conn->query("SELECT * FROM inventory ORDER BY $sort $order LIMIT $start, $limit");
?>

<!-- Records info and entries filter -->
<div class="table-controls">
    <div class="dataTables_info">
        Showing <?= $start + 1 ?> to <?= min($start + $limit, $totalRecords) ?> of <?= $totalRecords ?> entries
    </div>
    <div class="filter-box">
        <label for="limit">Show</label>
        <select id="limit" onchange="location.href='?page=1&limit=' + this.value">
            <option value="10" <?= $limit == 10 ? 'selected' : '' ?>>10</option>
            <option value="25" <?= $limit == 25 ? 'selected' : '' ?>>25</option>
            <option value="50" <?= $limit == 50 ? 'selected' : '' ?>>50</option>
            <option value="100" <?= $limit == 100 ? 'selected' : '' ?>>100</option>
        </select>
        <label for="limit">entries</label>
    </div>
</div>

<!-- Inventory Table -->
<table class="table" id="myTable">
    <thead>
        <tr>
            <th><a href="?sort=inventoryid&order=<?= $sort === 'inventoryid' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">ID</a></th>
            <th><a href="?sort=productcode&order=<?= $sort === 'productcode' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">Product Code</a></th>
            <th><a href="?sort=productname&order=<?= $sort === 'productname' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">Product Name</a></th>
            <th><a href="?sort=productcategory&order=<?= $sort === 'productcategory' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">Category</a></th>
            <th><a href="?sort=availablequantity&order=<?= $sort === 'availablequantity' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">Available Qty</a></th>
            <th><a href="?sort=onhandquantity&order=<?= $sort === 'onhandquantity' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">On Hand</a></th>
            <th><a href="?sort=dateupdated&order=<?= $sort === 'dateupdated' && $order === 'ASC' ? 'DESC' : 'ASC' ?>">Last Updated</a></th>
        </tr>
    </thead>
    <tbody>
        <?php
        if ($items->num_rows > 0) {
            while ($row = $items->fetch_assoc()) {
                $stockStatus = '';
                if ($row['availablequantity'] <= 0) {
                    $stockStatus = 'out-of-stock';
                } elseif ($row['availablequantity'] <= 10) {
                    $stockStatus = 'low-stock';
                }
                ?>
                <tr class="<?= $stockStatus ?>">
                    <td><?= htmlspecialchars($row['inventoryid']) ?></td>
                    <td><?= htmlspecialchars($row['productcode']) ?></td>
                    <td><?= htmlspecialchars($row['productname']) ?></td>
                    <td><?= htmlspecialchars($row['productcategory']) ?></td>
                    <td><?= htmlspecialchars($row['availablequantity']) ?></td>
                    <td><?= htmlspecialchars($row['onhandquantity']) ?></td>
                    <td><?= htmlspecialchars($row['dateupdated']) ?></td>
                </tr>
                <?php
            }
        } else {
            echo "<tr><td colspan='7' class='no-records'>No records found</td></tr>";
        }
        ?>
    </tbody>
</table>

<!-- Pagination -->
<div class="pagination-container">
    <ul class="pagination">
        <li><a href="?page=<?= max(1, $page - 1) ?>&limit=<?= $limit ?>&sort=<?= $sort ?>&order=<?= $order ?>" class="<?= $page == 1 ? 'disabled' : '' ?>">&laquo;</a></li>
        <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
            <li><a href="?page=<?= $i ?>&limit=<?= $limit ?>&sort=<?= $sort ?>&order=<?= $order ?>" class="<?= $page == $i ? 'active' : '' ?>"><?= $i ?></a></li>
        <?php endfor; ?>
        <li><a href="?page=<?= min($totalPages, $page + 1) ?>&limit=<?= $limit ?>&sort=<?= $sort ?>&order=<?= $order ?>" class="<?= $page == $totalPages ? 'disabled' : '' ?>">&raquo;</a></li>
    </ul>
</div>
