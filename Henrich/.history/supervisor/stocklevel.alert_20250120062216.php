...
if (!isset($conn)) {
    require '../in/dbconnect.php';
}

// Get low stock and out of stock items
$lowStockItems = $conn->query("SELECT * FROM inventory WHERE availablequantity > 0 AND availablequantity <= 10 ORDER BY availablequantity ASC");
$outOfStockItems = $conn->query("SELECT * FROM inventory WHERE availablequantity = 0 ORDER BY productname ASC");
            ...
            <div class="alert-list">
                <?php while ($item = $lowStockItems->fetch_assoc()): ?>
                    <div class="alert-item low-stock">
                        ...
                        <span class="item-name"><?php echo htmlspecialchars($item['productname']); ?></span>
                        <span class="stock-count"><?php echo $item['availablequantity']; ?> units left</span>
                    </div>
                <?php endwhile; ?>
            </div>
        </div>
