...
try {
    // Get total records count early - update table name if different
    $total_records_query = "SELECT COUNT(*) as count FROM customer";
    $total_records = $conn->query($total_records_query)->fetch_assoc()['count'];

    // Get today's active customers count - update column names if different
    $today_active_query = "SELECT COUNT(DISTINCT customer_id) as count FROM orders WHERE DATE(order_date) = CURDATE()";
...
$display_query = "SELECT * FROM customer LIMIT ? OFFSET ?";
// ...existing code...
?>
<!DOCTYPE html>
<html>
