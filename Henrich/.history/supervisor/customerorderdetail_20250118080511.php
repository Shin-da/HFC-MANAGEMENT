...
// Update order status
if (isset($_POST['updateStatus']) && isset($_POST['hid'])) {
    $update_hid = mysqli_real_escape_string($conn, $_POST['hid']);
    $status = mysqli_real_escape_string($conn, $_POST['status']);
    
    $update_sql = "UPDATE customerorder SET status = ?, datecompleted = NOW() WHERE hid = ?";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param("ss", $status, $update_hid);
    
    if ($stmt->execute()) {
        echo "<script>
            alert('Order status has been updated!');
            window.location.href = 'customerorderdetail.php?hid=" . $update_hid . "';
        </script>";
        exit;
    } else {
...
<body>
    ...
    <script>
        function updateOrderStatus(form, status) {
            Swal.fire({
