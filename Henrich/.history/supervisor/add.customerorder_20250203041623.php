<?php
require_once '../includes/config.php';  // Fix path
...
<script>
        ...
        function getProductInfo(select) {
            ...
            fetch(`./process/get_product_info.php?productcode=${encodeURIComponent(productCode)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        // Update all related fields
                        document.getElementById(`productname${rowId}`).value = data.productname;
                        document.getElementById(`unit_price${rowId}`).value = data.unit_price;
                        document.getElementById(`productweight${rowId}`).value = data.productweight;
                        document.getElementById(`availablequantity${rowId}`).value = data.availablequantity;
                        document.getElementById(`stockstatus${rowId}`).value = data.stock_status;
                        
                        // Reset quantity and recalculate totals
                        ...
                        // Update stock status visual indicator
                        const stockStatusInput = document.getElementById(`stockstatus${rowId}`);
                        if (stockStatusInput) {
                            stockStatusInput.className = `stock-status ${data.stock_status.toLowerCase().replace(' ', '-')}`;
                        }

                        // Show warning for low stock
                        if (data.availablequantity < 5 && data.availablequantity > 0) {
                            Swal.fire({
                                icon: 'warning',
                                title: 'Low Stock',
                                text: `Only ${data.availablequantity} items remaining in stock!`,
                                toast: true,
                                position: 'top-end',
                ...
                .catch(error => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Failed to fetch product information. Please try again.',
                        toast: true,
                        position: 'top-end',
                        showConfirmButton: false,
                        timer: 3000
        ...
        function formatProductSelection(product) {
            ...
            return $(`
                <span>
                    ...
                    <span style="color: ${statusColor}"> (${qty} packs)</span>
                </span>
            `);
        }
